...
class DataGenerator(tf.keras.utils.Sequence):
    def __init__(self, images_paths, labels, batch_size=8, dim = (128,128,3), shuffle=True,augment = False, seq=False, seed=1):
    ...
    def __getitem__(self, index):
        ...
        #x = np.array([(self.images_paths[i]) for i in indexes])
        if self.augment:
            x = self.augmentor(x)
